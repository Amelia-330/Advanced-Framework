...
//--------------------------------------------------------------
void ofApp::setup(){
    video.loadMovie( "try.mov" );  //Load the video file
...
void ofApp::draw(){
    ...
    if ( diffFloat.bAllocated ) {
        ...
        grayImage.draw( 0, 0, w/4, h/4 );
        diffFloat.draw( w/4 , 0, w/4, h/4 );
        bufferFloat.draw( 0, h/4, w/4, h/4 );

        //Draw the image motion areas
        //Shift and scale the coordinate system
        ofPushMatrix();
        ofTranslate( w/4+10, h/4+10 );
        ofScale( 0.2, 0.2 );

        //Draw bounding rectangle
        ofSetColor(0, 0, 0);
        ofNoFill();
        ofRect( 0, 0, w*1.2, h*1.2);
